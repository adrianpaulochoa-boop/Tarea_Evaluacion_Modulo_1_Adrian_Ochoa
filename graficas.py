...
# Genera gráficos interactivos para IPR, Perfil Hidrostático y Volumetría.
# =============================================================================
...
def graficar_ipr(pr, pb, j, pwf_usuario, q_usuario):
    """
    Construye la curva IPR completa evaluando presiones desde 0 hasta Pr
    y señala el punto de operación calculado por el usuario.
    """
    # 1. Generación de vector de presiones (eje Y)
    pwf_array = np.linspace(0, pr, 50)
    q_array = []
    
    # 2. Cálculo del caudal para cada punto de la curva
    ...
    # 3. Construcción del gráfico interactivo
    fig = go.Figure()
    
    # Trazo de la curva IPR completa
    ...
    # Marcador del punto operativo del usuario
    fig.add_trace(go.Scatter(
        x=[q_usuario], y=[pwf_usuario], 
        mode='markers', name='Punto Operativo', 
        marker=dict(color='#A3E635', size=12, symbol='circle', line=dict(color='#F1FAF7', width=2))
    ))
    
    # 4. Configuración de estilo alineada a la paleta global
    fig.update_layout(
...
def graficar_hidrostatica(mw, tvd_usuario, ph_usuario):
    """
    Genera un perfil de presión hidrostática versus profundidad (TVD).
    Invierte el eje Y para representar físicamente la profundidad del pozo.
    """
    # 1. Generación de vector de profundidad extendido ligeramente más allá del TVD del usuario
    tvd_array = np.linspace(0, tvd_usuario * 1.1, 20)
    ph_array = 0.052 * mw * tvd_array
    
    fig = go.Figure()
    
    # Línea de gradiente del lodo
    fig.add_trace(go.Scatter(
    ...
    # Marcador de la profundidad analizada
    fig.add_trace(go.Scatter(
        x=[ph_usuario], y=[tvd_usuario], 
        mode='markers', name='Profundidad Actual', 
        marker=dict(color='#A3E635', size=12, line=dict(color='#F1FAF7', width=2))
    ))
    
    # Configuración de ejes (Eje Y invertido)
    fig.update_layout(
...
def graficar_poes(poes_mmstb, volumen_recuperable_mmstb):
    """
    Crea un gráfico de barras apiladas para visualizar la proporción 
    del volumen recuperable respecto al POES total.
    """
